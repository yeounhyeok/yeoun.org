...
description: "NPM 워커 프로세스가 60초 간격으로 SIGSEGV로 죽으며 모든 서비스가 502로 떨어진 장애를 로그 교차 검증으로 추적한 기록. OpenResty/LuaJIT 누적 상태 + DNS 캐시 미스가 만든 크래시 루프와 v2.15.1 업그레이드로 해결한 과정을 정리했다."
lang: ko
draft: false
...
홈랩의 모든 서비스가 Cloudflare를 거쳐 Nginx Proxy Manager(이하 NPM)로 들어온다. NPM이 죽으면 블로그, Nextcloud, Immich, 음악 서버까지 전부 502가 뜬다. 이번 장애는 NPM 컨테이너 자체는 살아있었지만, nginx 워커 프로세스가 1분마다 SIGSEGV(시그널 11)로 죽어 나가면서 발생했다.
...
| 7/25 22:39 | DNS 에러 로그: `unexpected DNS response for n4200.soay-quail.ts.net` |
| 7/24 07:22 | DNS 에러 로그: `send() failed (111: Connection refused) while resolving, resolver: 127.0.0.11:53` |
...
### 1. 증상 파악: 502의 원인이 컨테이너가 아니라 워커였다

`yeoun.org`가 502를 반환했다. 첫 반응은 "백엔드 컨테이너가 죽었나?"였다. 하지만 astro-blog 컨테이너(n4200:8084)는 정상이었고, NPM → 백엔드 내부 요청도 200 OK를 반환했다. NPM 컨테이너 자체도 `Up 2 weeks` 상태였다.
...
### 2. 트리거 특정: Nextcloud 재생성이 시작점이었다

segfault가 처음 발생한 시각(16:43:57)과 그 직전에 일어난 작업을 대조했다. 12분 전(16:31)에 Nextcloud 컨테이너를 `docker compose up -d`로 재생성했다. 이 과정에서:

- 컨테이너가 재생성되면서 Docker IP가 변경되었다
- NPM의 DNS resolver(`127.0.0.11`, `valid=10s`)는 Docker embedded DNS를 통해 백엔드 IP를 resolving한다
- 10초마다 캐시가 갱신되지만, 이미 7/25부터 DNS 응답에 간헐적 오류가 있었다
...
### 3. 60초 주기 = Uptime Kuma 헬스체크

segfault가 60초마다 발생한다는 패턴을 발견했다. 이는 Uptime Kuma의 헬스체크 주기와 정확히 일치한다.
...
워커가 요청을 받고 응답을 보내기 전에 죽었다는 것은 access log에 해당 시간의 기록이 없는 것으로 확인했다. 워커는 요청 수신 → 처리 중(또는 직후)에 segfault로 종료되고, 마스터 프로세스가 새 워커를 fork하지만, 다음 60초 주기 요청이 오면 같은 경로로 다시 크래시하는 루프가 12분간 이어졌다.

### 4. 외부 요인 배제

segfault 구간(16:43~16:55) 동안 외부 요청이 있었는지 확인했다.

- yeoun.org 접근 로그: 외부(Cloudflare) 요청 0건
- 다른 vhost(immich, music 등)의 외부 요청도 0건
- 내부 Tailscale 클라이언트(Immich 폰 업로드, Navidrome 웹 플레이어)는 다른 vhost에서 정상 처리

즉, 외부 트래픽이 segfault를 유발한 것이 아니라, 내부 헬스체크 요청이 손상된 워커를 타격한 것이다.
