...
</code></pre><p>하지만 운영자가 실제로 알고 싶은 건 이것이다.</p><ul><li>앱 컨테이너가 죽었나?</li><li>프록시가 죽었나?</li><li>DNS가 깨졌나?</li><li>DB가 죽었나?</li><li>방금 누가 재시작했나?</li><li>지금 복구됐나?</li><li>다음에 재발하지 않으려면 뭘 바꿔야 하나?</li></ul><p>그래서 알림을 <strong>최종 메시지</strong>가 아니라 <strong>에이전트 실행 트리거</strong>로 바꿨다.</p>
<h2 id="k3s로-가도-에이전트가-필요한-이유">k3s로 가도 에이전트가 필요한 이유</h2>
<p>처음에는 이 구조를 “컨테이너가 죽으면 다시 켜는 자동화” 정도로 오해하기 쉽다. 만약 그게 전부라면 Kubernetes나 k3s의 self-healing만으로도 상당 부분 해결된다. 하지만 실제 홈랩 장애는 Pod 하나가 죽는 수준에서 끝나지 않았다.</p>
<p>6월 1일 DNS 장애에서 문제는 애플리케이션 컨테이너가 아니라 더 아래 레이어였다. root crontab의 <code>safe-reboot.sh</code>가 Docker daemon을 직접 내렸고, <code>unless-stopped</code> 정책이 기대와 다르게 복구되지 않았으며, 별도로 Glances Docker plugin이 Docker stats API 경로를 계속 찌르다가 dockerd fatal crash까지 트리거했다. 이 상황에서는 k3s가 있어도 컨테이너 런타임과 호스트 OS가 흔들리기 때문에 Kubernetes control loop만으로는 원인을 고치기 어렵다.</p>
<p>즉 에이전트의 역할은 “쿠버네티스보다 똑똑한 재시작 버튼”이 아니다. 더 정확히는 <strong>Kubernetes native 기능이 닿지 않는 호스트 OS, 컨테이너 런타임, 네트워크, DNS 레이어까지 함께 읽고 상관관계를 만드는 SRE 보조자</strong>다.</p>
<pre><code class="language-text">Kubernetes / k3s
  - Pod 상태, Deployment, Service, Ingress 복구에 강함

Hermes Agent
  - journalctl, cron, Docker/containerd, DNS, tunnel, SSH, 과거 incident 기록까지 연결
  - 클러스터 바깥 원인을 추론하고 사람에게 조치 범위를 제안
</code></pre>
<p>따라서 k3s 전환 이후에도 에이전트 파이프라인은 사라지는 것이 아니라 역할이 바뀐다. Pod 재시작은 k3s에 맡기고, 에이전트는 <strong>클러스터 아래와 바깥에서 발생하는 장애를 진단하고 격리하는 계층</strong>으로 남는 편이 맞다.</p>
<h2 id="hermes-webhook-%EC%84%A4%EC%A0%95">Hermes Webhook 설정</h2><p>Hermes Gateway의 webhook 플랫폼을 활성화하고, Uptime Kuma 컨테이너가 접근 가능한 Docker bridge gateway IP에 바인딩했다.</p><pre><code class="language-yaml">platforms:
  webhook:
    enabled: true
    extra:
      host: "172.24.0.1"
