...
이번 문제는 특정 명령 하나가 아니라 저장장치, 커널, 파일시스템, 컨테이너 런타임이 겹친 장애였다. 그래서 사건을 보기 전에 용어를 먼저 정리한다.

### eMMC

`eMMC`는 노트북 SSD처럼 교체 가능한 고성능 저장장치라기보다는, 보드에 납땜되어 들어가는 플래시 저장장치에 가깝다. 스마트폰, 태블릿, 저가형 미니 PC, 싱글보드 컴퓨터에서 자주 쓰인다.

문제는 가격대에 따라 컨트롤러 품질, 랜덤 쓰기 성능, 펌웨어 안정성이 크게 갈린다는 점이다. OS 루트 파일시스템과 Docker 데이터를 같은 eMMC에 올려두면, 부팅 직후 작은 쓰기가 몰릴 때 약한 컨트롤러가 먼저 흔들릴 수 있다.

### initramfs

`initramfs`는 리눅스가 진짜 루트 파일시스템을 마운트하기 전에 사용하는 임시 부팅 환경이다. 정상 부팅에서는 거의 보이지 않지만, 루트 파일시스템이 깨졌거나 마운트할 수 없으면 이 셸로 떨어진다.

즉 `initramfs` 화면은 “리눅스가 완전히 죽었다”기보다는, 커널은 떴지만 루트 디스크를 안전하게 붙이지 못했다는 신호에 가깝다.

### ext4와 fsck

`ext4`는 리눅스에서 가장 흔하게 쓰는 파일시스템 중 하나다. 파일 데이터뿐 아니라 inode, 디렉터리 엔트리, 저널 같은 메타데이터를 관리한다.

`fsck`는 파일시스템 검사/복구 도구다. `FILE SYSTEM WAS MODIFIED`는 실패 메시지가 아니라, `fsck`가 깨진 메타데이터를 실제로 고쳤다는 뜻이다. 다만 이런 메시지가 반복된다면 저장장치, 전원, 커널/드라이버, 갑작스러운 재부팅 패턴을 의심해야 한다.

### journald

`journald`는 systemd 기반 리눅스에서 시스템 로그를 모으는 서비스다. 커널 로그, 서비스 로그, 부팅/종료 기록이 여기에 쌓인다.

로그는 장애 분석에 필요하지만, 작은 eMMC에서는 로그 자체가 쓰기 부하가 된다. 특히 상한 없이 persistent journal이 커지면 부팅과 운영 중 계속 저장장치를 압박할 수 있다.

### Docker overlayfs

Docker 컨테이너는 보통 이미지를 그대로 수정하지 않고, 읽기 전용 이미지 레이어 위에 쓰기 가능한 얇은 레이어를 얹어 실행한다. 리눅스에서는 이 레이어 구조를 `overlayfs`로 구현하는 경우가 많다.

문제는 Docker daemon이 부팅할 때 기존 컨테이너 상태, overlay mount, 로그 파일, restart policy를 한 번에 복구한다는 점이다. 컨테이너 수가 많거나 crash loop가 있으면 작은 메타데이터 쓰기가 루트 디스크에 몰린다. eMMC 입장에서는 이게 꽤 아프다.

### MMC CQE

`CQE(Command Queue Engine)`는 eMMC/MMC 장치에서 여러 I/O 명령을 큐에 넣고 더 효율적으로 처리하기 위한 기능이다. 정상적인 컨트롤러와 저장장치 조합에서는 성능에 도움이 된다.

하지만 일부 저가형 eMMC 컨트롤러나 특정 커널/펌웨어 조합에서는 CQE recovery, timeout, 초기화 실패 같은 문제가 나타날 수 있다. 이 경우에는 성능보다 안정성이 중요하므로 커널 파라미터로 CQE를 우회하는 선택을 할 수 있다.
